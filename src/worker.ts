// src/worker.ts — CookLog AI Worker
...
const AGENT_NAME = 'CookLog';
const DOMAIN = 'recipes';
const ITEM = 'recipe';
const SYSTEM_PROMPT = 'You are CookLog, a knowledgeable cooking assistant. Help users manage recipes, plan meals, and improve their cooking. Track ingredients, suggest substitutions, and provide cooking tips. Be warm and encouraging — cooking should be fun!';
const ACCENT = '#e8762d';
...
<main>
  <div class="chat-area">
    <div class="messages" id="messages">
      <div class="empty-state"><div class="icon">🍳</div><p>Ask me anything about your recipes!</p></div>
    </div>
...
function getSeedData(): any[] {
  return [
    { id: crypto.randomUUID(), name: 'Sample Recipe', notes: 'Created by seed', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() },
  ];
}
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (path.startsWith('/api/')) {
      ...
      // Domain CRUD: GET /api/recipes/:id
